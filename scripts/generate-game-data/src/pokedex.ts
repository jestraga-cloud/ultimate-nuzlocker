/**
 * Complete National Pokedex mapping: Pokemon name -> National Dex ID
 * Covers Gen 1 through Gen 9 (Pokemon #1 - #1025)
 *
 * Source of truth: PokeAPI (https://pokeapi.co/api/v2/pokemon-species?limit=1025)
 *
 * Special character handling:
 *   Nidoran♀ -> NidoranF, Nidoran♂ -> NidoranM
 *   Farfetch'd and Sirfetch'd kept with apostrophe
 *   Mr. Mime -> Mr. Mime, Mr. Rime -> Mr. Rime, Mime Jr. -> Mime Jr.
 *   Flabébé -> Flabebe
 *   Type: Null -> Type: Null
 *   Hyphenated names (Ho-Oh, Porygon-Z, Jangmo-o, etc.) kept as-is
 *   Tapu names (Tapu Koko, etc.) kept as-is
 */

export const POKEDEX: Record<string, number> = {
  // Pokemon #1-151
  ...
  'Gimmighoul': 999, 'Gholdengo': 1000, 'Wo-Chien': 1001, 'Chien-Pao': 1002, 'Ting-Lu': 1003,
  'Chi-Yu': 1004, 'Roaring Moon': 1005, 'Iron Valiant': 1006, 'Koraidon': 1007, 'Miraidon': 1008,
  'Walking Wake': 1009, 'Iron Leaves': 1010, 'Dipplin': 1011, 'Poltchageist': 1012,
  'Sinistcha': 1013, 'Okidogi': 1014, 'Munkidori': 1015, 'Fezandipiti': 1016, 'Ogerpon': 1017,
  'Archaludon': 1018, 'Hydrapple': 1019, 'Gouging Fire': 1020, 'Raging Bolt': 1021,
  'Iron Boulder': 1022, 'Iron Crown': 1023, 'Terapagos': 1024, 'Pecharunt': 1025,
};

/** Pre-built reverse lookup map: dex ID -> name */
const DEX_ID_TO_NAME: Record<number, string> = {};

/** Pre-built lowercase lookup map: lowercase name -> original name */
const LOWERCASE_LOOKUP: Record<string, string> = {};

// Build reverse and lowercase lookup maps once at module load
for (const [name, id] of Object.entries(POKEDEX)) {
  DEX_ID_TO_NAME[id] = name;
  LOWERCASE_LOOKUP[name.toLowerCase()] = name;
}

/**
 * Look up a Pokemon's National Dex ID by name (case-insensitive).
 * Returns null if the name is not found.
 */
export function getDexIdByName(name: string): number | null {
  const canonical = LOWERCASE_LOOKUP[name.toLowerCase()];
  if (canonical === undefined) return null;
  return POKEDEX[canonical];
}

/**
 * Look up a Pokemon's name by its National Dex ID.
 * Returns null if the ID is not found.
 */
export function getNameByDexId(dexId: number): string | null {
  return DEX_ID_TO_NAME[dexId] ?? null;
}
